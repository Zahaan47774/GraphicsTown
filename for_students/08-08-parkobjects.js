...
export class GrCarousel extends GrObject {
  ...
  constructor(params = {}) {
    ...
    let horses = new T.SphereGeometry(0.4);
    let horses_mat = new T.MeshStandardMaterial({
      color: "red",
      metalness: 0.8,
      roughness: 0.5
    });
    ...
    for (let i = 0; i < num_poles; i++) {
      ...
      let loader = new OBJLoader();
                loader.load( "./images/obj/seahorse.obj", function ( object ) {

                   object.traverse( function ( child ) {
                             if ( child instanceof T.Mesh ) {
                                  child.material.color.setHex(Math.random() * 0xFFFFFF << 0);
                                 }
                             } );
                    as_group.add(object);               
            } );
      as_group.scale.set(0.8, 0.8, 0.8);
  ...
  stepWorld(step, timeOfDay) {
    ...
    this.whole_ob.rotateY(0.05 * t);
    for(let i = 0; i<10;i++){
    if (t < 0.1 || t > 0.9) {
      this.objhorse[i].position.y = 0.5;
    }
    else {
      this.objhorse[i].position.y = 0.5 + 10 * (0.16 - (0.5 - t) * (0.5 - t));
    }
  }
}
}
